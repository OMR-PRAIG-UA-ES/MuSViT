...
class SMTFoundationModelForCausalLM(PreTrainedModel):
    ...
    def __init__(self, config:SMTFoundationConfig):
        ...
        self.adaptor = nn.Conv1d(in_channels=adaptor_in, out_channels=256, kernel_size=1, stride=1)
    ...
    def forward_decoder(self, encoder_output, y_pred):
        ...
        encoder_output = encoder_output.reshape((b, 768, int(ln**(1/2.)), int(ln**(1/2.)))) 
    ...
    def forward(self, x, y_pred, labels=None):
        ...
        output = self.forward_decoder(x.permute(0,2,1).contiguous(), y_pred)
